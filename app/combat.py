...
def player_attack(state, enemy, sound):
    player = state['player']
...
def start_combat(state, enemy_type) -> None:
    ...
    if enemy['tags'] == ['normal']:
        if state['player']['round'] < 4:
            growl1.play()
        else:
            sprinter1.play()
        render_text(state, zombie)
    elif enemy['tags'] == ['special'] and enemy['name'] == ['Hell Hound']:
        render_text(state, hell_hound)

    elif enemy['tags'] == ['special'] and enemy['name'] == ['Vermin']:
        render_text(state, vermin)    
    ...
    while enemy['base_hp'] > 0 and state['player']['base_hp'] > 0:
        ...
        if player_choice == 'S':
            player_attack(state, enemy, sound=gun_1911.play())
